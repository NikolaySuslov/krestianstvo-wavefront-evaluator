...
// BASE_DELAY must be large enough that the full self-loop chain (gen 0..4) plus
// all child branches fit within CYCLE ticks with spread across the full cycle.
// Total time budget per chain: BASE × (1 + r + r² + r³ + r⁴) ≈ BASE × 3.3 (for r=0.618)
// To fill CYCLE=60t: BASE ≈ 60 / (DEPTH × 3.3) ≈ 3.6t → use 4φ ≈ 6.47t (irrational)
// BASE = π-2 ≈ 1.1416 — transcendental, no algebraic relation to √2,√3,√5.
// Products (π-2)×r^n stay irrational for all r in MAPS (all algebraic irrationals).
// CYCLE set to match actual beat spread: ~20t content → 25t cycle gives slight breathing room.
const FRACTAL_BASE_DELAY = 2.1415926535; // (π-2)×2 ≈ 2.142t — halved so chain fits in shorter cycle
const FRACTAL_MIN_DELAY  = 0.1;          // floor 0.1t = 5ms
const FRACTAL_CYCLE      = 8;            // new cycle every 8 ticks ≈ 0.4s → ~150 cycles/min
const FRACTAL_DECAY      = 0.1;
// Ratios: all algebraic irrationals, none is 1/φ (which would cancel 4φ base to integer).
// √2-1≈0.4142, √3-1≈0.7320, √5-2≈0.2360 — mutually incommensurable.
const FRACTAL_IFS_MAPS   = [0.4142135623, 0.6180339887, 0.7320508075]; // √2-1, 1/φ, √3-1
...
const fractalHeartbeatWorldProgram = makeIfsClock({
  ...
  genCap:    6,
  // Lorenz initial condition — called once per cycle after RNG is reseeded.
  // Consumes exactly 3 RNG values.
  onCycle: `(cycleCount, p, W) => {
    const r1 = W.rng.next(), r2 = W.rng.next(), r3 = W.rng.next();
    return { lx: (r1 - 0.5) * 2, ly: 1 + (r2 - 0.5) * 2, lz: (r3 - 0.5) * 2 };
  }`,
  // Lorenz RK4 step — called at every beat. Consumes 0 RNG values (determinism safe).
  // Returns updated (lx, ly, lz) merged into tickEvent and carried in next beat payload.
  onBeat: `(p, ctx, W) => {
...
function makeFractalHeartbeatRenderer(core) {
  ...
  return (world, peerId, containerId, sendCursorMove) => {
    ...
    let _lastDelay  = Array(FRACTAL_DEPTH).fill(0); // last known delay per depth from tickEvents
    ...
    // Beat log: accumulated beats for the last seen cycle, for the beat-log panel
    let _logCycleId = -1;
    let _cycleLog   = []; // [{ depth, wt, delay }]
    // IFS attractor (log-delay/depth): all beat points ever seen — never cleared
    const _ifsPoints = [];
    // Wavefront attractor (log-delay / phase wt%1): 2D scale-phase plane — never cleared
    const _wfaPoints = [];
    // Lorenz attractor: (lx, lz) points accumulated across all cycles — never cleared
    const _lorenzPoints = [];
    let   _ifsPrevDelay = -1;
    let   _lorenzIdx    = 0;  // global beat counter for warmup filtering
    let   _lorenzCycle  = -1; // tracks cycle resets
    ...
    return () => {
      ...
      if (lt !== _historyLt) {
        ...
        for (let d = 0; d < FRACTAL_DEPTH; d++) {
          // Use last known delay for this depth if available, else fall back to base/2^d
          const periodTicks = (_lastDelay[d] > 0) ? _lastDelay[d] : FRACTAL_BASE_DELAY / Math.pow(2, d);
        ...
        for (const { d, wt, delay, lx, ly, lz } of tickEvs) {
          ...
          // Wavefront attractor: scale-phase plane (log delay × wt%1)
          const ax = Math.min(1, Math.max(0, xRaw));
          const ay = wt % 1;
          _wfaPoints.push({ x: ax, y: ay, col: DEPTH_COLORS[d] ?? '#888' });
          // Lorenz: accumulate (lx, lz) for butterfly plot — color by depth
      ...
      if (!root) {
        ...
        // Beat log: last N beats, one per line — lets you verify IFS spacing without waterfall
        // root.appendChild(label('BEAT LOG (last cycle)', { fontSize: '9px', color: '#333', marginTop: '10px', marginBottom: '3px', letterSpacing: '0.5px' }));
        // const beatLog = mk('div', {
        //   fontFamily: 'ui-monospace,monospace', fontSize: '8px', lineHeight: '1.7',
        //   background: '#080810', borderRadius: '4px', padding: '6px 8px',
        //   color: '#555', height: '420px', overflowY: 'scroll',
        // });
        // beatLog.id = containerId + '-beatlog';
        // root.appendChild(beatLog);
      ...
      // Beat log panel: one line per beat in current cycle, sorted by wallTime
      // const beatLog = document.getElementById(containerId + '-beatlog');
      // if (beatLog && _cycleLog.length > 0) {
      //   const sorted = _cycleLog.slice().sort((a, b) => a.wt - b.wt);
      //   const cycleBase = sorted[0]?.wt ?? 0;
      //   beatLog.innerHTML = sorted.map(({ depth, wt, delay }) => {
      //     const col    = DEPTH_COLORS[depth] ?? '#888';
      //     const offset = (wt - cycleBase).toFixed(4);
      //     const bar    = '█'.repeat(depth + 1);
      //     return `<span style="color:${col}">${bar} D${depth}</span>` +
      //            ` wt=<span style="color:#666">${wt.toFixed(3)}</span>` +
      //            ` +${offset}t` +
      //            ` delay=<span style="color:#444">${delay.toFixed(4)}t</span>`;
      //   }).join('<br>');
      // }

      // IFS attractor canvas — accumulates all beat points, never clears
      // x-axis: wt mod 1 (fractional sub-tick position, 0..1)
      // y-axis: depth (0=top .. DEPTH-1=bottom)
      ...
      if (ifsCanvas && _ifsPoints.length > 0) {
        ...
        // Incrementally paint only new points — never redraws old ones
        ...
        for (let i = from; i < _ifsPoints.length; i++) {
          ...
          c2d.beginPath(); c2d.arc(px, py, 4, 0, Math.PI * 2); c2d.fill();
          c2d.globalAlpha = 0.75;
          c2d.beginPath(); c2d.arc(px, py, 1.2, 0, Math.PI * 2); c2d.fill();
        }
        ifsCanvas._drawn = _ifsPoints.length;
        c2d.globalAlpha = 1;
      }

      // Lorenz attractor canvas — (lx, lz) butterfly, accumulated across all cycles
      // Lorenz X range ≈ -25..25, Z range ≈ 0..50
      ...
      if (artCanvas && _lorenzPoints.length > 0) {
        ...
        // Fixed bounds matching the Lorenz attractor's known range (clamped at accumulation)
        const bx0 = -22, bx1 = 22, bz0 = 5, bz1 = 47;
        const toCanvasX = lx => PAD + ((lx - bx0) / (bx1 - bx0)) * AW;
        const toCanvasY = lz => PAD + AH - ((lz - bz0) / (bz1 - bz0)) * AH;
        // Incremental draw — bounds are fixed so old points remain correct
      ...
      // Wavefront attractor: scale-phase plane — incremental, fixed bounds [0,1]×[0,1]
...
// ── Rössler Attractor via IFS Clock ──────────────────────────────────────────
// Rössler ODE: dx=-y-z, dy=x+ay, dz=b+z(x-c)  a=0.2 b=0.2 c=5.7
// Projection: (x, y) plane shows the characteristic single-lobe spiral.
// IFS clock drives RK4 integration — same makeIfsClock, different onBeat.
...
function makeRosslerRenderer(core) {
  ...
  return (world, peerId, containerId) => {
    ...
    // Return the actual render function called by Renkon each tick
    return () => {
      const f = world.getNodeState('fractal');
      if (!f) return;

      // Accumulate new (rx, ry) points — Rössler (x,y) attractor bounds
      ...
      const AW = CW - PAD*2, AH = CH - PAD*2;
      // Rössler attractor: x∈[-11,13], y∈[-12,10] — asymmetric, fitted to fill canvas
...
const FRACTAL_BASE_DELAY_1 = 8.3;   // outer-tick units; D0=8.3t D1=4.15t D2=2.075t D3=1.0375t D4=0.52t — D0–D3 each cross tick boundaries independently; D4 sub-tick only
const FRACTAL_MIN_DELAY_1  = 0.0001; // stop cascading below this threshold
const FRACTAL_CYCLE_1      = 200;   // global ticks between cascade restarts (~10s at 50ms/tick)
const FRACTAL_DECAY_1      = 0.03;  // energy decay per global tick (~33 ticks to fade, spans the longer cycle)
...
function makeFractalHeartbeatRenderer1(core) {
  ...
  return (world, peerId, containerId, sendCursorMove) => {
    ...
    return () => {
      ...
      const _cursorFrac = (lt % FRACTAL_CYCLE_1) / FRACTAL_CYCLE_1;
      ...
      */
      ...
      if (tl1Canvas) {
        ...
        const RING_W = (R_MAX - R_MIN) / (FRACTAL_DEPTH_1 - 1) * 0.38; // arc band half-width
        const ARC_W  = 0.04 * Math.PI; // angular half-width of each mark
        const c2d    = tl1Canvas.getContext('2d');
        c2d.clearRect(0, 0, CW, CH);
