  ...
  const mediumU1 = Behaviors.collect(
    {
      ...
      // MEMORY-OCCLUSION dedicated world state (part of the world, NOT last-verb-wins medVirt*): the CURRENT damage
      //   level + method (so every peer's damage slider/method reflect it, incl. on JOIN), and whether P2 holds a
      //   live plate + which (so the 'live plate' button state is replicated). These persist across other verbs.
      medDmgFrac: 0, medDmgMode: 7, medDmgSeq: 0,   // current occludebank damage level / method / a bump seq (change signal)
      medLivePlate: -1,                              // plate index living in P2 (−1 = none) — set by platelive, replicated
      medLensObj: 'letterA',                         // WHAT the pin holds: the probe geometry (dense glyph vs sparse light points). Dedicated persistent field so every peer + a joiner reflect it (the medVirt* fields are last-verb-wins and unreliable for reflection).
    ...
    (state, pulse) => {
      ...
      return W.reduce(s, pulse, 'mediumU1', {
        ...
        mediumVirt: (s, p) => { const mode = (p && ['relift','swap','mirror','mux','store','recall','hold','leak','ear','boot','kill','go','tex','aphase','cseed','selfclock','edge','refamp','lenstau','recordraw','recvia','viaphi','lensview','lensset','coevo','unpin','fieldmix','recalla','descgo','wabs','autoc','tempo','xatt','recallx','recallo','occludebank','platelive','virtt','pinhold','lensobj','selfatt','synthspec','turbo','linonly'].includes(p.mode)) ? p.mode : 'record';   // selfatt: the FIELD-AS-ATTRACTOR door (amp = digest bars; 0 = revert to probe pin) — the symbol's identity migrates from the operator into the field   // lensobj: WHAT the pin holds (the probe geometry — dense glyph vs sparse light points). The lock is unchanged; only its target. Replicated (the pin target is in regH).   // pinhold: steps a living recalled slot stays driven by its plate attractor (0 = forever) — replicated, it scales the pin (in regH)   // virtt: the ±T hologram DEPTH (amp = T steps) — replicated (store/recall leg uses it)   // platelive: lift a stored plate into a LIVING 𝔸-slot (the memory as an evolving soliton, not a frozen image)   // occludebank: DAMAGE a stored plate in place (memory-side occlusion — the corrupted-memory recovery test)   // fieldmix: toggle the edge's SECOND coupling layer (attractor field-mixing) — edge always couples PHASE, this gates the shape-bleed   // recallo: OCCLUDED recall — bind a MASKED cue (occlude mode/frac/block/seed), lift the WHOLE plate ("break the hologram, reconstruct the whole scene")   // xatt: exact spectral attractor (toggle) · recallx: shift-aware recall (finds+relocates a moved moment)   // tempo: the WORLD PROPER-TIME divisor (amp 1–8) — the §7.44 law as a replicated dial: when supply < demand (shared GPU, weak peer) slow matter's clock so the backlog can't spiral   // autoc: ⌀PDE continuous compilation — the mirror re-compiles the register's envelope every amp bars (0 = off)   // wabs: ⌀PDE — close the whole register into abstract holography (amp=1) / materialize back into the medium (amp=0)   // recalla (medium-u1): descriptor-only 𝔸-recall — the abstractive-holography read (closed-form cue⊗bank on the ℂ* plates, no field) · descgo: 𝔸-slot transport target (the descriptor leash — movement with 0 grid steps)   // coevo/unpin (medium-u1): the ⟲Einstein-loop toggle + the lensU1⇄lensC1 governance switch — REPLICATED because they gate the leash advance and gx/gy is in regH (a per-peer flag would fork the register AND the field)   // recordraw = ⎙virt with NO birth-default toggles (snap⇢V); recvia = record THROUGH a lens (u-register step 4 — amp = the birth angle φ); viaphi/lensview = replicated bar5 dials (step 5); lensset = the extended readOp components (metric/gauge sector, read-side)
          const e = { seq: (s.medVirtSeq|0) + 1, time: (s.time ?? 0), mode,
            // recall PLACEMENT: [x,y] in leash coords, clamped like gx/gy. Rides the ENTRY (medVirtLog is what the
            // drain reads). null = keep the classic behaviour (stored pos for recall@, cue pos for recall⇄).
            at: (Array.isArray(p.at) && p.at.length === 2) ? [Math.max(-24, Math.min(24, +p.at[0] || 0)), Math.max(-24, Math.min(24, +p.at[1] || 0))] : null,
            obj: (mode === 'lensobj' && ['ring','point','pair','grid','cross','blob','letterA','depthscene','lightpts'].includes(String(p.obj))) ? String(p.obj) : (s.medLensObj ?? 'letterA'),   // lensobj: the probe geometry the pin holds — MUST ride the ENTRY (medVirtLog is what the drain reads), not only the state. lightpts = Gaussian light points (finite waist, below the ring kKnee → coherent)
            scale: (mode === 'recall' || mode === 'recallx' || mode === 'recallo') ? ((p.scale === 'coarse' || p.scale === 'fine') ? p.scale : 'all') : (s.medVirtScale ?? 'all'),   // recall SCALE BAND (coarse/fine/all) — the tier-selective ±T lift; replicated (changes V's field = in eH)
            holoMode: (mode === 'recallo' || mode === 'occludebank') ? ([1,2,3,5,6,7,8].includes(p.holoMode|0) ? (p.holoMode|0) : 7) : (s.medVirtHoloMode ?? 0),   // occluder mode (1 LP · 2 HP · 3 conj · 5 box · 6 half · 7 rand-zero · 8 rand-noise) — shared by recallo (cue) + occludebank (plate); replicated
            frac: (mode === 'recallo' || mode === 'occludebank') ? Math.max(0, Math.min(1, +p.frac || 0)) : (s.medVirtFrac ?? 0),   // occluder fraction removed / aperture radius
            block: (mode === 'recallo' || mode === 'occludebank') ? Math.max(1, Math.min(64, p.block|0 || 8)) : (s.medVirtBlock ?? 0),   // random-block size px (modes 7/8)
            seed: (mode === 'recallo' || mode === 'occludebank') ? Math.max(0, Math.min(9999, +p.seed || 0)) : (s.medVirtSeed ?? 0),   // reshuffle the random mask pattern
            ...
            gx: (mode === 'go' || mode === 'descgo' || mode === 'tex' || mode === 'cseed' || mode === 'edge') ? Math.max(-24, Math.min(24, +p.gx || 0)) : (mode === 'occludebank' || mode === 'platelive') ? Math.max(-1, Math.min(63, p.gx|0)) : (mode === 'synthspec') ? Math.max(0, Math.min(400, p.gx|0)) : (s.medVirtGx ?? 0),   // occludebank/platelive: gx = the PLATE INDEX (−1 = the last stored)   // tex/cseed reuse gx/gy as integer cycle counts (kx,ky); edge as the slot pair (a,b); descgo as the 𝔸-slot leash target
            gy: (mode === 'go' || mode === 'descgo' || mode === 'tex' || mode === 'cseed' || mode === 'edge') ? Math.max(-24, Math.min(24, +p.gy || 0)) : (s.medVirtGy ?? 0),
            amp: (mode === 'tex' || mode === 'cseed') ? Math.max(0, Math.min(2, +p.amp || 0)) : (mode === 'aphase' || mode === 'recvia' || mode === 'viaphi') ? Math.max(-Math.PI, Math.min(Math.PI, +p.amp || 0)) : (mode === 'lenstau') ? Math.max(-0.5, Math.min(0.5, +p.amp || 0)) : (mode === 'refamp') ? Math.max(0, Math.min(3, (p && p.amp != null) ? +p.amp : 1)) : (mode === 'coevo' || mode === 'unpin' || mode === 'fieldmix' || mode === 'wabs' || mode === 'xatt' || mode === 'turbo') ? ((+p.amp || 0) ? 1 : 0) : (mode === 'linonly') ? Math.max(0, Math.min(2, Math.round(+p.amp || 0))) : (mode === 'autoc') ? Math.max(0, Math.min(64, Math.round(+p.amp || 0))) : (mode === 'tempo') ? Math.max(1, Math.min(8, Math.round(+p.amp || 1))) : (mode === 'virtt') ? Math.max(1, Math.min(500, Math.round(+p.amp || 16))) : (mode === 'pinhold') ? Math.max(0, Math.min(4000, Math.round(+p.amp || 0))) : (mode === 'selfatt') ? Math.max(0, Math.min(64, Math.round(+p.amp || 0))) : (mode === 'synthspec') ? Math.max(0, Math.min(4096, Math.round(+p.amp || 0))) : (s.medVirtAmp ?? 0),   // virtt: the ±T hologram depth (1–256)   // the TOGGLE verbs (coevo/unpin/wabs) carry their own 0/1 — inheriting the previous amp made the toggle depend on verb history (latent stale-amp bug)   // tex phase depth / cseed amp / aphase increment / recvia birth angle (±π) / refamp pin-amplitude multiplier (0–3)
            src: (mode === 'go' || mode === 'descgo') ? ((p && (p.src === 'P1' || p.src === 'P2')) ? p.src : 'V')
               : (mode === 'aphase' || mode === 'refamp' || mode === 'lenstau' || mode === 'lensset' || mode === 'record' || mode === 'recvia' || mode === 'store' || mode === 'recall' || mode === 'recallx' || mode === 'recallo' || mode === 'recalla' || mode === 'platelive') ? ((p && (p.src === 'V' || p.src === 'P1' || p.src === 'P2' || p.src === 'W')) ? p.src : 'W')   // slot-explicit verbs (stamped-carrier principle — must not depend on the gaze OR the previous verb): aphase/refamp/LENSTAU (per-slot ω)/lensset + the MEMORY verbs (record/recall targets — inheriting medVirtSrc sent every P1/P2 recall to a stale slot, the amp-inheritance bug class at the src field)
          ...
          // DEDICATED world fields (persist across other verbs → the UI can read them back reliably, incl. on join):
          //   occludebank updates the current damage level/method; platelive records which plate lives in P2.
          const _dmgFrac = (mode === 'occludebank') ? e.frac : (s.medDmgFrac ?? 0);
          const _dmgMode = (mode === 'occludebank') ? e.holoMode : (s.medDmgMode ?? 7);
          const _dmgSeq  = (mode === 'occludebank') ? ((s.medDmgSeq | 0) + 1) : (s.medDmgSeq | 0);   // bump on each damage (change signal for the slider even at the same frac)
          const _livePl  = (mode === 'platelive') ? Math.max(0, e.gx | 0) : (s.medLivePlate ?? -1);
          const _lensObj = e.obj;   // the entry already clamped/inherited it (see e.obj above) — keep state and entry identical
          return { ...s, medVirtSeq: e.seq, medVirtTime: e.time, medVirtMode: e.mode, medVirtLeak: e.leak, medVirtScale: e.scale, medVirtSlot: e.slot, medVirtGx: e.gx, medVirtGy: e.gy, medVirtAmp: e.amp, medVirtSrc: e.src, medVirtHoloMode: e.holoMode, medVirtFrac: e.frac, medVirtBlock: e.block, medVirtSeed: e.seed,
            medDmgFrac: _dmgFrac, medDmgMode: _dmgMode, medDmgSeq: _dmgSeq, medLivePlate: _livePl, medLensObj: _lensObj,
            medVirtLog: [...(Array.isArray(s.medVirtLog) ? s.medVirtLog : []), e].slice(-64) }; },   // last 64 verbs (was 16 — the "sometimes not work" cause: a setup burst ⎙virt+hold+mux+store+boot×2+attPhase+edge×6 ≈ 12 verbs; a peer/pull >16 seq behind found the older unseen verbs already SLICED OFF the log → silently dropped. 64 covers bursts + a sweep)   // leak = the W→V coupling κ · mirror = toggle V's drive · mux = toggle §7.44 clock-phase time-sharing
