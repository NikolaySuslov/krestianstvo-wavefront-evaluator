...
import { makeObserverBank, normalizeVirtEvent, applySettingsVerb, makeStepClock, makeCouplingStore, muxClocks, chainMeter, makeRippleClock, makeSelfHost } from '../medium-core.js';
  ...
  const med = Behaviors.collect(
    ...
    (state, pulse) => {
      ...
      return W.reduce(st, pulse, 'med', {
        ...
        verb: (s, p, ctx) => {
          const MODES = ['record','recvia','aphase','lenstau','refamp','viaphi','store','recall','boot','kill','edge','tempo','beatmode','stretch'];
          ...
          const e = { seq: (s.seq|0) + 1, time: s.time, mode,
            amp: (mode === 'tempo') ? cl(p.amp, 1, 96, 19) : (mode === 'beatmode') ? cl(p.amp, 0, 2, 1)
...
function makeRhythmRenderer(core) {
  ...
  return (world, peerId, containerId, sendCursorMove, injectEvent) => {
    ...
    let inhabit = -1;                                                    // −1 = COORDINATE (the ensemble, mux-dilated — outside view, default); 0..3 = INHABIT that worldline: the inhabitant's beats are the master pulse and the OTHER slots re-time to it (slower ones thin out — dilation seen from inside). PURE RENDER (audio re-timing at the ear), NEVER touches the engine's k → determinism intact. Why not "absolute full-speed"? That would need MORE beats/sec than the shared engine produces — impossible without time-travel; the honest inside-view is the relative one.
    const nSlots = () => 1 + (voices[1] ? 1 : 0) + (voices[2] ? 1 : 0) + (voices[3] ? 1 : 0);
    ...
    const appVerb = (vq, kk) => {
      ...
      if (vq.mode === 'aphase') { const si = SNI[vq.src] ?? 0;
      ...
      else if (vq.mode === 'beatmode') { const si = SNI[vq.src] ?? 0; beatMode[si] = ['ripple', 'rhythm', 'host'][Math.round(vq.amp) | 0] || 'ripple';
        ...
        console.log(`[RHY] beat source ${SLOTN[si]} = ${beatMode[si]} atStep=${kk} — the kernel never knows why a voice ticks${beatMode[si] === 'host' ? ' (self-host matter: rich ripple from M↔O, not the chord)' : ''}`); }
      ...
      else if (vq.mode === 'store') { if (voices[0]) { plates.push({ p: Array.from(voices[0]), a: Array.from(chords[0]), lop: { ...ops[0] }, bw: tauK.beatsOf('W') ?? 0 });
        if (plates.length > 4) plates.shift(); console.log(`[RHY] store — the moment's voice banked ${plates.length}/4 (frame ∠${lensU1.angle(ops[0]).toFixed(3)}, bw=${tauK.beatsOf('W') ?? 0})`); } }
      else if (vq.mode === 'recall') { if (plates.length && voices[0]) {
        let best = 0, bc = -1; for (let i = 0; i < plates.length; i++) { const c = corrAbs(voices[0], Float64Array.from(plates[i].p)); if (c > bc) { bc = c; best = i; } }
        const pl = plates[best]; voices[1] = Float64Array.from(pl.p); chords[1] = Float64Array.from(pl.a); E0[1] = energy(voices[1]); Object.assign(ops[1], lensU1.id()); chPh[1] = NaN; chEff[1] = null; kp[1] = 0;
        const dN = (tauK.beatsOf('W') ?? 0) - (pl.bw ?? 0);
        console.log(`[RHY] recall → plate ${best + 1} (cue⊗=${bc.toFixed(3)}) sounds again as V atStep=${kk}`);
        console.log(`[RECALL-∠] plate frame ∠=${lensU1.angle(pl.lop).toFixed(3)} vs W now ∠=${lensU1.angle(ops[0]).toFixed(3)} → Δ=${lensU1.wrap(lensU1.angle(ops[0]) - lensU1.angle(pl.lop)).toFixed(3)} rad · Δτ_W=${dN} beats${ops[0].omega ? ` → ω·Δτ=${lensU1.wrap(ops[0].omega * dN).toFixed(3)} predicted` : ''} — the remembered moment returns AGED (you can hear the detune)`); } }
    ...
    const takeSnap = () => ({ k, seq: seen, clk: { c0: clk.c0, rate: clk.rate, ratePrev: clk.ratePrev },
      ...
      E0: [...E0], kp: [...kp], beatMode: [...beatMode], tempo: [...tempo], stretch: [...stretch], beatAcc: [...beatAcc], ripple: ripple.map((r) => r.save()), host: host.map((h) => h.save()), plates: plates.map((pl) => ({ ...pl, lop: { ...pl.lop } })) });
    const restoreSnap = (s) => { k = s.k | 0; seen = s.seq | 0;
      ...
      plates.length = 0; for (const pl of (s.plates || [])) plates.push({ p: [...pl.p], a: [...pl.a], lop: { ...pl.lop }, bw: pl.bw | 0 });
    ...
    //    labeled as such) — dilation is only PERCEPTIBLE against an undilated reference; as worlds join, the whole
    //    ensemble audibly falls behind the click.
    ...
    const clickToggle = () => { clickOn = !clickOn; nextClick = 0; console.log(`[RHY] wall click ${clickOn ? 'ON — coordinate time, 1/s of WALL (the undilated reference the ensemble falls behind)' : 'OFF'}`); return clickOn; };
    ...
    const inhabitCycle = () => {                                         // COORDINATE → W → V → P1 → P2 → COORDINATE. A frame choice at the observer boundary (render-only, peer-local)
      ...
      console.log(`[RHY] inhabit = ${inhabit < 0 ? 'COORDINATE (the ensemble, mux-dilated — the outside view)' : `${SLOTN[inhabit]} (INSIDE: its beats are the master pulse; the others re-time to it — slower ones thin out, dilation from within)`}`);
      return inhabit; };
    ...
    const pluck = (i) => { if (!AC || !voices[i]) return;
      ...
      const ang = lensU1.wrap(lensU1.angle(ops[i]));                     // the register angle → detune: ±200 cents per π — precession is a GLISSANDO over beats
      const cents = ang * (200 / Math.PI), f0 = BASEHZ[i] * Math.pow(2, cents / 1200);
      const t0 = AC.currentTime, e = Math.min(1, energy(voices[i]) / (E0[i] || 1));
      for (let h = 0; h < H; h++) { const amp = Math.hypot(voices[i][h*2], voices[i][h*2+1]);
    ...
    globalThis.rhy = {
      ...
      regPhase: () => ({ beats: SLOTN.map((nm) => tauK.beatsOf(nm) ?? 0), tau: SLOTN.map((nm) => +tauK.tauOf(nm).toFixed(3)), step: k, kH: tauK.hash(), vH: voices[0] ? hashF(voices[0]) : null }),
      ...
      beatMode: (slot = 'V', m = 'rhythm') => inj({ mode: 'beatmode', src: slot, amp: { ripple: 0, rhythm: 1, host: 2 }[m] ?? 1 }),
    ...
    const mkUI = (root) => {
      root.innerHTML = `
        ...
        <div style="margin-top:4px;font-size:9px;color:#654;">console: rhy.sound() · rhy.rec(φ) · rhy.tempo('V',N) · rhy.lensTau(ω) · rhy.beatMode('V','ripple') · rhy.chainRead() · rhy.store()/recall() · rhy.edge(a,b,κ) · rhy.checkpoint()</div>`;
      ...
      const sPhi = sld('φ', -3.14, 3.14, 0.05, 0.5, (v) => inj({ mode: 'viaphi', amp: v }));
      const sOm = sld('ω', -0.5, 0.5, 0.01, 0, (v) => globalThis.rhy.lensTau(v));                      // GLOBAL (one verb sets all slots — see lenstau)
      ...
      btn('beat:src', () => { const order = ['ripple', 'rhythm', 'host']; const cur = order.indexOf(beatMode[SLOTN.indexOf(slot)]);
        globalThis.rhy.beatMode(slot, order[(cur + 1) % 3]); });   // cycle ripple → rhythm → host
      ...
      return { cvs, sPhi, sOm, sBeta, sTmp, sStr, slotSel: () => slot, st: document.getElementById(`${containerId}-st`), roster: document.getElementById(`${containerId}-roster`) };
    };
    const face = (i) => { const { ctx, lab } = els.cvs[i]; const R = 42;
      ...
      const modeTag = beatMode[i] === 'rhythm' ? `rhythm·${tempo[i].toFixed(2)}` : beatMode[i] === 'host' ? 'host·M↔O' : `ripple:${stretch[i].toFixed(2)}`;
    ...
    return () => {
      ...
      if (els) { for (let i = 0; i < 4; i++) face(i);
        els.sPhi.set(dials.viaPhi); els.sOm.set(ops[0].omega);           // reflect replicated state onto the dials (bar5 law)
        const si = Math.max(0, SLOTN.indexOf(els.slotSel())); els.sBeta.set(ops[si].beta); els.sTmp.set(tempo[si]); els.sStr.set(stretch[si]);
