...
//  wire schema, the settings-verb table, the coupling store (the capture law), the mux two-clock law,
//  and the chain meter — and provides ONLY its own physics: a 16×16 CPU Schrödinger toy per slot
//  (superpose-toward-att pin + dispersion + energy cap). No GPU, no soliton engine, no history.
//
...
//  tempos + lensTau(ω) = the twin experiment on a metronome, read by the register (chainRead pred/mdl,
//  [RECALL-∠] plate stamps).
//
...
import { makeObserverBank, normalizeVirtEvent, applySettingsVerb, makeStepClock, makeCouplingStore, muxClocks, chainMeter, makeRippleClock } from '../medium-core.js';
...
const BETA = 0.055, CDISP = 0.28;    // pin gain per proper step · dispersion strength
    ...
    (state, pulse) => {
      ...
      return W.reduce(st, pulse, 'med', {
        ...
        verb: (s, p, ctx) => {
          ...
          const MODES = ['record','recvia','aphase','lenstau','lensset','refamp','viaphi','lensview','store','recall','boot','kill','edge','tempo','beatmode'];
...
function makeObserversRenderer(core) {
  ...
  return (world, peerId, containerId, sendCursorMove, injectEvent) => {
    ...
    const plates = [];                                                   // { p, a, lop, bw } — observer-stamped memory
    ...
    const takeSnap = () => ({ k, seq: seen,
      ...
      plates: plates.map((pl) => ({ p: Array.from(pl.p), a: Array.from(pl.a), lop: { ...pl.lop }, bw: pl.bw })) });
    const restoreSnap = (s) => {
      ...
      plates.length = 0; for (const pl of (s.plates || [])) plates.push({ p: Float64Array.from(pl.p), a: Float64Array.from(pl.a), lop: { ...pl.lop }, bw: pl.bw | 0 });
      booted = true;
    ...
    const nSlots = () => 1 + (fields[1] ? 1 : 0) + (fields[2] ? 1 : 0) + (fields[3] ? 1 : 0);
    ...
    const stepSlot = (i, kk) => { const f = fields[i]; if (!f) return;
      rebuildAtt(i);
      const beta = BETA * (ops[i].beta || 1), a = attEff[i];
      if (a) for (let j = 0; j < f.length; j++) f[j] += beta * a[j];    // the pin (superpose toward the reference)
      // dispersion: ψ += i·c·∇²ψ on the torus (split real/imag; scratch holds the pre-step field)
      scratch.set(f);
      for (let y = 0; y < G; y++) for (let x = 0; x < G; x++) { const j = (y * G + x) * 2;
        const xm = (x + G - 1) % G, xp = (x + 1) % G, ym = (y + G - 1) % G, yp = (y + 1) % G;
        const lr = scratch[(y * G + xm) * 2] + scratch[(y * G + xp) * 2] + scratch[(ym * G + x) * 2] + scratch[(yp * G + x) * 2] - 4 * scratch[j];
        const li = scratch[(y * G + xm) * 2 + 1] + scratch[(y * G + xp) * 2 + 1] + scratch[(ym * G + x) * 2 + 1] + scratch[(yp * G + x) * 2 + 1] - 4 * scratch[j + 1];
        f[j] += -CDISP * li; f[j + 1] += CDISP * lr; }
      ...
      if (beat) { tauK.beat(SLOTN[i], kp[i], kk); ops[i].phase = wrap2pi(ops[i].phase + (ops[i].omega || 0)); }   // precession per OWN beat (W-convention: phase absorbs it)
    ...
    const appVerb = (vq, kk) => {                                        // the thin app's own optics table (the medium keeps its own — physics is the app's)
      ...
      else if (vq.mode === 'store') { if (fields[0]) { plates.push({ p: fields[0].slice(), a: attBase[0].slice(), lop: { ...ops[0] }, bw: tauK.beatsOf('W') ?? 0 });
        if (plates.length > 4) plates.shift(); console.log(`[OBS] store — plate ${plates.length}/4 banked (frame ∠${lensU1.angle(ops[0]).toFixed(3)}, bw=${tauK.beatsOf('W') ?? 0}) atStep=${kk}`); } }
      else if (vq.mode === 'recall') { if (plates.length && fields[0]) {
        let best = 0, bc = -1; for (let i = 0; i < plates.length; i++) { const c = corrAbs(fields[0], plates[i].p); if (c > bc) { bc = c; best = i; } }
        const pl = plates[best]; fields[1] = pl.p.slice(); attBase[1] = pl.a.slice(); E0[1] = energy(fields[1]); Object.assign(ops[1], lensU1.id()); attPh[1] = 0; attEff[1] = null; kp[1] = 0;
        const dN = (tauK.beatsOf('W') ?? 0) - (pl.bw ?? 0);
        console.log(`[OBS] recall → plate ${best + 1} (cue⊗=${bc.toFixed(3)}) lifted into V atStep=${kk}`);
        console.log(`[RECALL-∠] plate frame ∠=${lensU1.angle(pl.lop).toFixed(3)} vs W now ∠=${lensU1.angle(ops[0]).toFixed(3)} → observer shift Δ=${lensU1.wrap(lensU1.angle(ops[0]) - lensU1.angle(pl.lop)).toFixed(3)} rad · Δτ_W=${dN} beats${ops[0].omega ? ` → ω·Δτ=${lensU1.wrap(ops[0].omega * dN).toFixed(3)} predicted` : ''}`); } }
    ...
    const advance = (time) => { if (!booted) { const m0 = world.getNodeState('med'); if (m0?.snap) restoreSnap(m0.snap); else seed(); }
      ...
      try { while (done < 4000) {
        ...
        stepSlot(ph, k);                                                 // the mux time-shares the STEP BUDGET: only ph's slot advances (each world at 1/nSl proper rate — §7.44)
    ...
    globalThis.obs = { chainRead, chainSee, lensOps,
      regPhase: () => ({ beats: SLOTN.map((nm) => tauK.beatsOf(nm) ?? 0), tau: SLOTN.map((nm) => +tauK.tauOf(nm).toFixed(3)), step: k, kH: tauK.hash(), wH: fields[0] ? hashF(fields[0]) : null }),
    ...
    const mkUI = (root) => {
      root.innerHTML = `
        ...
        <div style="margin-top:4px;font-size:9px;color:#456;">console: obs.chainRead() · obs.chainSee() · obs.lensOps() · obs.regPhase() · obs.rec(φ) · obs.lensTau(ω) · obs.lensSet('V',{rot}) · obs.tempo('V',N) · obs.beatMode('V','rhythm') · obs.store()/recall() · obs.edge(a,b,κ)</div>`;
      ...
      const sOm = sld('ω', -0.5, 0.5, 0.01, 0, (v) => globalThis.obs.lensTau(v));                       // GLOBAL
      ...
      return { cvs, lv, sPhi, sOm, sRot, sBeta, sTmp, slotSel: () => slot, st: document.getElementById(`${containerId}-st`), roster: document.getElementById(`${containerId}-roster`) };
    };
    const paint = () => { if (!els) return;
      ...
      const si2 = Math.max(0, SLOTN.indexOf(els.slotSel()));
